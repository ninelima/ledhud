...
void GLPFD::renderCalibrate()
{
  GLfloat z;//, pixPerDegree;

  z = zfloat;

  QFont fn("Helvetica", 24, QFont::Normal);
  QFontMetrics fm = fontMetrics();
  QString t( "Calibrating...");
  qglColor( QColor( "red" ) );
  QGLWidget::renderText(0 - 1.5*fm.width(t) , 0  , z, t, fn, 2000 ) ;
}
...
void GLPFD::renderFixedHorizonMarkers()
{
  ...
  qglColor( QColor( "white" ) );
    glVertex3f( -6.0 * pixPerDegree, 0.0, z);
    glVertex3f( -4.0 * pixPerDegree, 0.0, z);
    glVertex3f( -2.0 * pixPerDegree, -2.0 * pixPerDegree, z);
...
void GLPFD::renderPitchMarkers()
{
	...
	//b2 QFont fn("Helvetica", 10, QFont::Bold);
	QFont fn("Helvetica", 10, QFont::Normal);
	QFontMetrics fm = fontMetrics();
  ...
  for (i = 90; i > 0; i=i-10) {
      ...
      glVertex3f( -outerTic, iPix - .03 * pixW2, z);
		glEnd();
    //QGLWidget::renderText( -0.13 * pixW2 - fm.width(t) - cos(rollRotation/57.29) * fm.width(t)/2,
    //                       iPix - fm.ascent() / 2 + sin(rollRotation/57.29) * fm.ascent()/2,  z, t, fn, 2000 ) ;
    QGLWidget::renderText( -0.13 * pixW2 - fm.width(t) - icos(rollRotation) * fm.width(t)/2,
                           iPix - fm.ascent() / 2 + isin(rollRotation) * fm.ascent()/2,  z, t, fn, 2000 ) ;
  ...
  for (i = 9; i >= 6; i=i-1) {
	...
	// horizon line - longer
	glBegin(GL_LINES);
    //glVertex3f( -0.3  * pixW2, 0.0, z);
    //glVertex3f( 0.3  * pixW2, 0.0, z);
    glVertex3f( -0.95  * pixW2, 0.0, z);
    glVertex3f( 0.95  * pixW2, 0.0, z);
  glEnd();
...
/*!
	Set the pitch
*/
void GLPFD::setPitch( float degrees )
{
  pitch = (GLfloat) degrees;
...
/*!
	Set the roll angle
*/
void GLPFD::setRoll( float degrees )
{
  //roll = (GLfloat)(degrees % 360);
  roll = (GLfloat) degrees;
  rollRotation = roll;
	updateGL();
}
