...
void GLPFD::renderFixedASIMarkers()
{
  ...
  QGLWidget::renderText ( (GLint) ( -0.85 * pixH2 - 1.5 * fm.width(t)), -fm.height()/2 + 1, z+.1, t, font, 2000 ) ;
	// Mask over the moving tape for the value display box
	qglColor( QColor( "black" ) );
	glBegin(GL_QUADS);
    glVertex3f( -1.05 * pixH2, -fm.height(), z+.1);
    glVertex3f( -1.05 * pixH2, fm.height(), z+.1);
    ...
    qglColor( QColor( "white" ) );
    glVertex3f( -1.05 * pixH2, -fm.height(), z+.2);
    glVertex3f( -1.05 * pixH2, fm.height(), z+.2);
...
void GLPFD::renderIASMarkers()
{
	...
	//font = QFont("Fixed", 12, QFont::Bold);
	font = QFont("Fixed", 12, QFont::Normal);
	QFontMetrics fm = fontMetrics();
  ...
  qglColor( QColor( "lightgray" ) );

	for (i = IASMaxDisp; i >=0; i=i-10) {
    if (i < IASValue-30) continue;
    if (i > IASValue+30) continue;
  ...
  if (displayTerrain) {
    ...
    midTic = -0.75 * pixH2;													// Put tape under the tics, w/ VsO-Vfe bar narrower than minor tics
