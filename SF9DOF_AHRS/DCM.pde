...
void DriftCorrection(void)
{
  ...
  //b2
  //AccMagnitude = 1; // testing
  GMagnitude = AccMagnitude;
  //b2
  ...
  //b2
  if ( abs(AccMagnitude - 1.0) < 0.005 ) {
    // do full correction if there is no acceleration (about 5 deg bank error)
    digitalWrite(STATUS_LED, HIGH);  
    digitalWrite(STATUS_LED, HIGH);  
    Kp_RollPitch =  p_RollPitch; 
    Ki_RollPitch =  i_RollPitch; 
  }
  else 
  if ( abs(AccMagnitude - 1.0) < 0.075 ) { //0.0005
    // Only correct little if we are not accelerating too much in any axis 
    digitalWrite(STATUS_LED, LOW);  
    digitalWrite(STATUS_LED, HIGH);  
    Kp_RollPitch =  p_RollPitch/50;
    Ki_RollPitch =  i_RollPitch/50;
  }
  else {
    // Do no correcting
    digitalWrite(STATUS_LED, LOW);  
...
void MUpdate(void)
{
  ...
  // b2
  // 1m/s = 1.943844 kts
  Velocity += Acc_V[0] / (float) GRAVITY / 10.0; // ~speed gs
  ...
  Update_M[1][1] =  0;
  Update_M[1][2] = -G_Dt * Gyro_V[0];   //-x 
  Update_M[2][0] = -G_Dt * Gyro_V[1];   //-y
  Update_M[2][1] =  G_Dt * Gyro_V[0];   //x
  Update_M[2][2] =  0;
#endif
...
void RateOfTurn(void)
{
  static float _Yaw;
  static long LastClockmS;
  // Rate of turn  
  
  Rot = 1000*(_Yaw - Yaw) / (millis() - LastClockmS);
  _Yaw = Yaw;
  LastClockmS = millis();
